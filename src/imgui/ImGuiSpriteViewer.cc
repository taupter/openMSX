                               ...
                               float zoom, std::span<uint32_t, 16> palette, ImTextureID patternTex)
{
	...
	if (mode == 1) {
		...
		ImGui::ImageWithBg(patternTex, zoom * gl::vec2{float(size)}, {u1, v1}, {u2, v2}, {}, getColor(color));
	} else {
		...
		for (auto y : xrange(size)) {
			...
			ImGui::ImageWithBg(patternTex, zoom * gl::vec2{float(size), 1.0f}, {u1, v1}, {u2, v2}, {}, getColor(color));
			v1 = v2;
		}
	}
}
